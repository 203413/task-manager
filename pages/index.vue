<template>
    <div>
        <div class="card-header">
            ...
            <h5>Taks list
                <NuxtLink to="/tasks/create">Add task</NuxtLink>
            </h5>
        </div>
        <div class="card-body">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <tr v-for="(task, index) in tasks" :key="index">
                        ...
                        <td>{{ task.due_date }}</td>
                        <td>
                            <NuxtLink :to="`/tasks/${task.id}`" class="btn btn-success btn-sm mx-2">Edit</NuxtLink>
                        </td>
                        <td><button type="button" @click="$event => deleteTask($event, task.id)"
                                class="btn btn-danger btn-sm mx-2">Delete</button></td>
                    </tr>
                </tbody>
            </table>
        </div>
...
export default {
    ...
    methods: {
        ...
        deleteTask(event, taskID) {
            console.log('holaaa');
            const url = 'https://ecsdevapi.nextline.mx/vdev/tasks-challenge/tasks/' + taskID;
